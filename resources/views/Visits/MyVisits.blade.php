<x-layouts.app>
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">Overzicht van jouw geplande, actieve en afgeronde bezoeken.</p>
        </div>
        <div class="flex flex-wrap gap-2">
        @if(in_array(auth()->user()?->role, ['admin', 'employee'], true))
            <a href="{{ route('visits.index') }}" class="px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-700 text-white">Alle bezoeken</a>
            <a href="{{ route('visits.create') }}" class="px-4 py-2 rounded-md bg-blue-600 hover:bg-blue-700 text-white">Nieuw bezoek</a>
        @endif
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="border-b border-gray-200 dark:border-gray-700 p-4 flex flex-wrap gap-2">
            <a href="{{ route('visits.myvisits') }}" class="px-3 py-1.5 rounded-full text-sm border {{ request('status') ? 'border-gray-200 dark:border-gray-700' : 'border-blue-600 text-blue-600' }}">Alles</a>
            <a href="{{ route('visits.myvisits', ['status' => 'planned']) }}" class="px-3 py-1.5 rounded-full text-sm border {{ request('status') === 'planned' ? 'border-blue-600 text-blue-600' : 'border-gray-200 dark:border-gray-700' }}">Gepland</a>
            <a href="{{ route('visits.myvisits', ['status' => 'in']) }}" class="px-3 py-1.5 rounded-full text-sm border {{ request('status') === 'in' ? 'border-blue-600 text-blue-600' : 'border-gray-200 dark:border-gray-700' }}">Actief</a>
            <a href="{{ route('visits.myvisits', ['status' => 'out']) }}" class="px-3 py-1.5 rounded-full text-sm border {{ request('status') === 'out' ? 'border-blue-600 text-blue-600' : 'border-gray-200 dark:border-gray-700' }}">Afgerond</a>
        </div>
